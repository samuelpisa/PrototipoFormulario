...
public class DbHelper extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		String sql = String.format("create table %s(%s integer primary key autoincrement, %s text);", TABLE_QUESTIONS,QUESTION_ID,QUESTION_DESC);
		Log.d(TAG,"onCreate sql1: "+sql);
		db.execSQL(sql);
		
		sql = String.format(" create table %s(%s integer primary key autoincrement, %s text, %s int);", TABLE_ALTERNATIVES,ALTERNATIVE_ID,ALTERNATIVE_DESC,ALTERNATIVE_QUESTION_ID);
